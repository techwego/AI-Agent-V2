...
import { Mic, Loader2, Volume2, Radio, Video, Sparkles, ShieldCheck } from 'lucide-react';
...
const InteractiveVideoAvatar = ({ 
  ...
  avatarProfile = 'Sam · Library AI Human'
}) => {
  ...
  // Audio-driven lip-sync viseme extractor
  ...
  // Dynamic animation speed for fallback 3D avatar
  const animationSpeed = useMemo(() => {
    if (isListening) return '1.4';
    if (isSpeaking) return '1.2';
    if (isProcessing) return '1.6';
    return '1.0';
  ...
  return (
    <div className="relative flex flex-col items-center justify-center select-none py-1">
      ...
      >
        ...
        <div className={`relative w-48 h-48 sm:w-60 sm:h-60 rounded-full flex items-center justify-center transition-all duration-500 backdrop-blur-xl border overflow-hidden ${
          isListening 
            ...
            : isSpeaking 
              ...
              : isProcessing 
                ...
                : 'bg-gradient-to-b from-white/85 to-blue-50/40 border-slate-200/80 shadow-xl shadow-blue-900/10 group-hover:border-blue-300 group-hover:shadow-2xl group-hover:shadow-blue-500/20'
        }`}>
          
          {/* Specular Inner Glass Highlights */}
          <div className="absolute inset-2 rounded-full border border-white/60 pointer-events-none z-20" />
          <div className="absolute top-2 left-6 right-6 h-10 rounded-full bg-gradient-to-b from-white/60 to-transparent pointer-events-none z-20" />

          {/* WebRTC Real-Time Lip-Sync Video Stream Layer (Simli / HeyGen / Tavus) */}
          {isWebRTCConnected ? (
            <video 
              ref={videoRef}
              autoPlay
            ...
            /* Enterprise 3D Animated Character Avatar with Real-Time Lip Sync & Idle Blinking */
            <div className="w-36 h-36 sm:w-48 sm:h-48 flex items-center justify-center transform-gpu group-hover:scale-105 transition-transform duration-300 pointer-events-none relative z-10">
              <dotlottie-player 
              ...
              {/* Dynamic Lip-Sync Viseme Aperture Layer when Speaking */}
              {isSpeaking && viseme.mouthOpen > 0.05 && (
                <div 
                  className="absolute bottom-11 sm:bottom-14 w-6 h-3 bg-indigo-900/30 rounded-full blur-[1px] transition-all duration-75 pointer-events-none"
                  style={{
                    transform: `scale(${1 + viseme.mouthWide * 0.4}, ${1 + viseme.mouthOpen * 0.8})`,
                    opacity: Math.min(0.6, viseme.mouthOpen)
                  }}
                />
              )}
            </div>
          ...
          {/* Real-Time Status Orb Badge Overlay */}
          <div className="absolute -bottom-2.5 sm:-bottom-3 z-30 transition-all duration-300 group-hover:scale-105">
            {isListening ? (
              <div className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-gradient-to-r from-red-500 to-rose-600 text-white text-xs font-bold shadow-lg shadow-red-500/30 border border-white/40 animate-pulse">
                <Radio size={13} className="animate-spin text-white" />
                <span className="tracking-wide">Listening...</span>
              </div>
            ) : isProcessing ? (
              <div className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 text-white text-xs font-bold shadow-lg shadow-amber-500/30 border border-white/40">
                <Loader2 size={13} className="animate-spin text-white" />
                <span className="tracking-wide">Thinking...</span>
              </div>
            ) : isSpeaking ? (
              <div className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-xs font-bold shadow-lg shadow-indigo-500/30 border border-white/40">
                <Volume2 size={13} className="animate-bounce text-white" />
                <span className="tracking-wide">Speaking...</span>
              </div>
            ) : (
              <div className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 text-white text-xs font-bold shadow-lg shadow-blue-600/30 border border-white/40 group-hover:from-blue-700 group-hover:to-violet-700">
