...
const StatusIndicator = ({ state = 'IDLE' }) => {
  return (
    <div className="flex flex-col items-center justify-center h-9 select-none">
      ...
      {state === 'IDLE' && (
        <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/90 border border-slate-200/90 text-xs font-semibold text-slate-600 shadow-sm transition-all animate-[fadeInScale_0.2s_ease-out]">
      ...
      {state === 'LISTENING' && (
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-sky-50 border border-sky-300 text-xs font-bold text-sky-700 shadow-md shadow-sky-500/10 transition-all animate-[fadeInScale_0.2s_ease-out]">
      ...
      {(state === 'PROCESSING' || state === 'RETRIEVING' || state === 'GENERATING') && (
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-50 border border-amber-200 text-xs font-bold text-amber-900 shadow-sm transition-all animate-[fadeInScale_0.2s_ease-out]">
      ...
      {state === 'SPEAKING' && (
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-indigo-50 border border-indigo-200 text-xs font-bold text-indigo-700 shadow-sm transition-all animate-[fadeInScale_0.2s_ease-out]">
          <div className="flex items-center gap-0.5 h-3">
